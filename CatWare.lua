...
local Window = Library.CreateLib("CatWare 1.7", "RJTheme7")
...
Section:NewButton("Aim", "Bind : E",  function()
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode.E then
        ePressed = not ePressed
        if ePressed then
            Players.LocalPlayer:GetMouse().Icon = "rbxassetid://0"
...
Section:NewSlider("FOV", "Field of view.", 200, 70, function(c)
game.Workspace.CurrentCamera.FieldOfView = c
end)
...
Section:NewButton("Nut Hub", "This hub have other scripts", function()
...
local function onMouseEnter(mouse)
  local player = Players:GetPlayerFromCharacter(mouse.Target)
  if player then
    UserInputService:SetMouseBehavior(Enum.MouseBehavior.ForceLeftButtonDown)
  end
end

local function onMouseLeave(mouse)
  ...
  if inputObject.KeyCode == Enum.KeyCode.Space then
    ...
    while UserInputService:IsKeyDown(Enum.KeyCode.Space) do
      for _, player in ipairs(Players:GetPlayers()) do
        local character = player.Character
        if character then
          character.HumanoidRootPart.MouseEnter:Connect(function(mouse)
            onMouseEnter(mouse)
          end)
          character.HumanoidRootPart.MouseLeave:Connect(function(mouse)
            onMouseLeave(mouse)
          end)
        end
      end
...
Section:NewButton("Boost for Walk-Speed", "Boost for your walking speed.", function()
...
Section:NewButton("Noclip", "Allows you to pass through walls. Useful with tacticool.", function()
...
Section:NewButton("Infinity-Jump [F]", "Allows you to jump endlessly. May reset. F KEYBIND", function()
...
local Section = Tab:NewSection("Hubs , Functions : dimasterskyyy , pika")
